      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {applications.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm border p-12 text-center">
            ...
            <div className="flex gap-4 justify-center">
              <Link to="/jobs" className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors">
                Browse Jobs
              </Link>
              <Link to="/recommendations" className="bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors">
                View Recommendations
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            {applications.map(application => {
              ...
              return (
                <div key={application.id} className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow overflow-hidden">
                  
                  {/* Top Section - Job Info & Status */}
                  <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-6">
                    <div className="flex justify-between items-start mb-3">
                      <h3 className="text-xl font-bold text-gray-900 flex-1">
                        {job ? (
                          ...
                          <span className="text-gray-600">Job Post Unavailable</span>
                        )}
                      </h3>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getStatusBadge(application.status)}`}>
                        {application.status === 'pending' && '⏳'}
                    ...
                    {job ? (
                      <div className="space-y-2 text-sm text-gray-700">
                        <p><i className="fas fa-building mr-2 text-indigo-500"></i>{job.company?.name || 'Company not available'}</p>
                        <p><i className="fas fa-map-marker-alt mr-2 text-indigo-500"></i>{job.location}</p>
                        <div className="flex gap-4">
                          <span><i className="fas fa-briefcase mr-2 text-indigo-500"></i>{job.type}</span>
                          {job.salary && <span><i className="fas fa-dollar-sign mr-2 text-indigo-500"></i>{job.salary}</span>}
                        </div>
                      </div>
                    ) : (
                      <div className="text-sm text-gray-500 italic">
                        <p>This job posting may have been removed by the employer.</p>
                      </div>
                    )}

                    <div className="mt-4 pt-4 border-t border-indigo-100 flex items-center justify-between text-xs">
                      <span className="inline-flex items-center px-2 py-1 rounded-full bg-indigo-100 text-indigo-800 font-medium">
                        <i className="fas fa-paper-plane mr-1"></i>Applied {getTimeAgo(application.appliedAt)}
                      </span>
                      <span className="text-gray-500">{formatDate(application.appliedAt)}</span>
                    </div>
                  </div>

                  {/* Middle Section - Cover Letter & Skills */}
                  <div className="p-6 space-y-4">
                    {application.coverLetter && (
                      <div>
                        <h4 className="text-sm font-semibold text-gray-700 mb-2 flex items-center">
                          <i className="fas fa-file-text mr-2 text-gray-500"></i>Cover Letter
                        </h4>
                    ...
                    {application.resume && (
                      <div>
                        <h4 className="text-sm font-semibold text-gray-700 mb-2 flex items-center">
                          <i className="fas fa-file-pdf mr-2 text-gray-500"></i>Resume
                        </h4>
                        <a href={application.resume} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-800 text-sm font-medium">
                          View Submitted Resume →
                        </a>
                      </div>
                    )}
                    ...
                    {job?.requiredSkills && job.requiredSkills.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {job.requiredSkills.length > 4 && (
                            <span className="text-xs text-gray-500 px-3 py-1">+{job.requiredSkills.length - 4} more</span>
                          )}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="p-6 pt-0">
                    <div className="flex gap-3">
                      {job && (
                        <Link to={`/jobs/${job.id}`} className="flex-1 text-center border border-indigo-300 text-indigo-700 hover:bg-indigo-50 font-semibold py-2.5 px-4 rounded-lg transition-colors">
                          View Job Details
                        </Link>
                      )}
                      
                      {application.status === 'accepted' && (
                        <button className="flex-1 bg-green-600 text-white font-semibold py-2.5 px-4 rounded-lg cursor-default">
                          ✅ Congratulations!
                        </button>
                      )}
                      
                      {application.status === 'rejected' && (
                        <button className="flex-1 bg-gray-400 text-white font-semibold py-2.5 px-4 rounded-lg cursor-default">
                          💼 Keep Looking
                        </button>
                      )}
                    </div>
        ...
        {applications.length > 0 && (
          <div className="mt-12 bg-indigo-50 rounded-lg border border-indigo-200 p-8 text-center">
            <h3 className="text-lg font-semibold text-indigo-900 mb-2">Keep Building Your Career</h3>
            <p className="text-indigo-700 mb-4">Continue applying to more positions that match your skills and goals.</p>
            <div className="flex gap-4 justify-center">
              <Link to="/jobs" className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">
                Find More Jobs
              </Link>
              <Link to="/recommendations" className="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-6 rounded-lg transition-colors">
                View Recommendations
              </Link>
            </div>
          </div>
